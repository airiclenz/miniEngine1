...
unsigned long calculate_ramp_delay() {
  ...
  // reached max speed phase  
    ...
    switch(MOTOR_RAMP_TYPE) {
    case RAMPING_LINEAR:    res = x_scale * delay_delta;
                            break;
    case RAMPING_ATANGENS:  res = (atan(15.0 * x_scale) / 1.51) * delay_delta;   // more steep curve
                            break;
    case RAMPING_SINUS:     res = (((cos(PI * (x_scale - 1))) / 2) + 0.5) * delay_delta;
                            break;
